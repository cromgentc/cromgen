...
const formDefaults = {
  ...
  'vendor-management': { name: '', company: '', email: '', phone: '', sourcingType: '', sourcingRegion: '', sourcingLanguage: '', serviceCategory: '', serviceSubCategory: '', portfolio: '', experience: '', message: '', password: '', status: 'active' },
...
function createVendorPayload(form = {}) {
  const payload = {
    ...
    sourcingType: String(form.sourcingType || '').trim(),
    sourcingRegion: String(form.sourcingRegion || '').trim(),
    sourcingLanguage: String(form.sourcingLanguage || '').trim(),
    serviceCategory: String(form.serviceCategory || '').trim(),
    serviceSubCategory: String(form.serviceSubCategory || '').trim(),
...
function getModuleConfig(page, data, currentRole = 'admin') {
  ...
  if (['vendor-management'].includes(page)) {
    return {
      ...
      rows: data.vendors.map((vendor) => ({
        ...
        serviceSubCategory: vendor.serviceSubCategory,
        sourcingType: vendor.sourcingType,
        sourcingRegion: vendor.sourcingRegion,
        sourcingLanguage: vendor.sourcingLanguage,
        ...
        updatedAt: vendor.updatedAt ? formatDate(vendor.updatedAt) : '',
      })),
      columns: commonColumns(['code', 'name', 'company', 'email', 'phone', 'service', 'serviceSubCategory', 'sourcingType', 'sourcingRegion', 'sourcingLanguage', 'status', 'approvalRemark', 'reviewedAt', 'createdAt']),
  ...
  if (['applications', 'candidate-management', 'interview-management'].includes(page)) {
    return {
      type: 'applications',
      title: 'Applications',
...
function getFormFields(page, data = emptyData, currentRole = 'admin') {
  ...
  const map = {
    ...
    'vendor-management': [
      ...
      { name: 'sourcingType', label: 'User Sourcing', type: 'select', options: ['India', 'International'] },
      { name: 'sourcingRegion', label: 'Sourcing Region' },
      { name: 'sourcingLanguage', label: 'Primary Language' },
      { name: 'serviceCategory', label: 'Service Category' },
      { name: 'serviceSubCategory', label: 'Service Sub Category' },
    ...
    'withdraw-requests': [
      ...
      { name: 'ifscCode', label: 'IFSC Code', showWhen: (form) => form.method === 'Bank Account' },
    ...
    'security-settings': [
      ...
      { name: 'scope', label: 'Scope', type: 'select', options: ['Admin', 'Admin & Staff', 'Staff', 'Vendor', 'User', 'Candidate', 'All Users', 'Public Website', 'API Access', 'File Uploads', 'Finance', 'Project Management'] },
...
function commonColumns(keys) {
  const labels = {
    ...
    sourcingType: 'User Sourcing',
    sourcingRegion: 'Sourcing Region',
    sourcingLanguage: 'Primary Language',
    portfolio: 'Portfolio',
    message: 'Message',
    title: 'Title',
    client: 'Client',
